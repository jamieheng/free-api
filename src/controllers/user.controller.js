...
const getAllUsers = async (req, res) => {
	try {
		...
		// Summarize user counts by department and job
		const summary = users.reduce(
			(acc, user) => {
				if (user.department) {
					acc.departments[user.department.name] =
						(acc.departments[user.department.name] || 0) + 1;
				}
				if (user.job) {
					acc.jobs[user.job.title] = (acc.jobs[user.job.title] || 0) + 1;
				}
				acc.total += 1;
				return acc;
			},
			{ total: 0, departments: {}, jobs: {} }
		);
